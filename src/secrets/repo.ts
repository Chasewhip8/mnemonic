				...
				Effect.gen(function* () {
					...
					Effect.catchAll((error) =>
						Effect.gen(function* () {
							...
							return { success: false, error: 'Failed to set secret' } as { success: boolean; error?: string };
						})
					)
				),
				...
				Effect.gen(function* () {
					...
					Effect.catchAll((error) =>
						Effect.gen(function* () {
							...
							return { success: false, error: 'Failed to delete secret' } as { success: boolean; error?: string };
						})
					)
				),
