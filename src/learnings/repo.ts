...
export class LearningsRepo extends Effect.Service<LearningsRepo>()(
	"LearningsRepo",
	{
		effect: Effect.gen(function* () {
				...
				Effect.gen(function* () {
					...
					if (format === "prompt") {
						const prompt = learnings
							.map(
								(learning) => `When ${learning.trigger}, ${learning.learning}`,
							)
							.join("\n");
			...
			const injectTrace = (
				...
				return Effect.gen(function* () {
					...
					const aboveThreshold = candidates.filter(
						(candidate) => candidate.passed_threshold,
					).length;
			...
			const getLearnings = (filter?: { scope?: string; limit?: number }) =>
				Effect.gen(function* () {
					...
					const queryBuilder = filter?.limit
						? scopedQuery.limit(filter.limit)
						: scopedQuery;
					...
					return results.map((row) =>
						convertSqlLearningRow(row as unknown as LearningRow),
					);
				}).pipe(
					Effect.catchAll((error) =>
						Effect.gen(function* () {
			...
			const deleteLearnings = (filters: DeleteLearningsFilters) =>
				Effect.gen(function* () {
					...
					if (filters.confidence_lt != null) {
						conditions.push(
							drizzleSql`${schema.learnings.confidence} < ${filters.confidence_lt}`,
						);
					}
					...
					if (filters.not_recalled_in_days != null) {
						if (filters.not_recalled_in_days === 0) {
							conditions.push(
								drizzleSql`${schema.learnings.lastRecalledAt} IS NULL`,
							);
						} else {
