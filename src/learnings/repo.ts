...
import { and, desc, eq, inArray, sql as drizzleSql, type SQL } from 'drizzle-orm';
	...
	effect: Effect.gen(function* () {
					...
					yield* Effect.tryPromise({
						try: () =>
							db
								...
								.where(inArray(schema.learnings.id, learnings.map((learning) => learning.id))),
						catch: (error) => error,
					});
				}
		...
		const query = (
			...
			Effect.gen(function* () {
				const filteredScopes = filterScopesByPriority(scopes);
				if (filteredScopes.length === 0) {
					return { learnings: [] as Array<Learning>, hits: {} as Record<string, number> };
				}
		...
		const deleteLearnings = (filters: DeleteLearningsFilters) =>
			Effect.gen(function* () {
				...
				const toDelete = yield* Effect.tryPromise({
					try: () => db.select({ id: schema.learnings.id }).from(schema.learnings).where(whereClause),
					catch: (error) => error,
				});
