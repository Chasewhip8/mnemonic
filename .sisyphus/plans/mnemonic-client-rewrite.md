# Rewrite mnemonic-client with HttpApiClient

## TL;DR

> **Quick Summary**: Replace the 257-line manual client with ~25 lines using `HttpApiClient.make(Api)`. All types, endpoints, error handling, and auth derive automatically from the existing server API definition. Co-locate `src/client.ts` with the API definitions; `packages/mnemonic-client/` becomes a thin publishing wrapper with tsup bundling + tree-shaking.
> 
> **Deliverables**:
> - `src/client.ts` — MnemonicClient Effect.Service wrapping HttpApiClient.make(Api)
> - Rewritten `packages/mnemonic-client/` — new build config, re-export, updated deps
...
### Original Request
Redo `packages/mnemonic-client` entirely. Use the HttpApi client for automatic type derivation. Stop redefining types. Make the client incredibly simple. Effect-only (drop Promise wrapper). Expose all endpoints.

### Interview Summary
**Key Discussions**:
- **Sharing strategy**: Co-locate `src/client.ts` with server API definitions. `packages/mnemonic-client/` is just a publishing wrapper. tsup tree-shakes server-only deps.
...
- `packages/mnemonic-client/src/index.ts` — rewritten (thin re-export)
- `packages/mnemonic-client/package.json` — updated deps, build script, version
- `packages/mnemonic-client/tsconfig.json` — updated for cross-package import
- `packages/mnemonic-client/README.md` — updated for Effect-only API
- `packages/mnemonic-client/test/client.test.ts` — minimal smoke test

### Definition of Done
- [ ] `tsc --noEmit` passes for root project (src/client.ts valid)
- [ ] `bun run build` in `packages/mnemonic-client/` succeeds
- [ ] `dist/` excludes server-only code (no drizzle, libsql, huggingface)
- [ ] `bun test` in `packages/mnemonic-client/` passes (smoke test)
- [ ] Client exposes all 4 groups with all 20 endpoints (type-level verification)

### Must Have
- MnemonicClient as `Effect.Service` wrapping `HttpApiClient.make(Api)` with `effect:` (NOT `scoped:`)
- `dependencies: [FetchHttpClient.layer]` in service definition
- Config-driven: `MNEMONIC_URL` (default `http://localhost:8787`), `MNEMONIC_API_KEY` (optional)
...
- NO Promise-based wrapper (`mnemonic()` function) — Effect-only
...
### Test Decision
- **Infrastructure exists**: YES (vitest in packages/mnemonic-client)
...
├── Task 2: Reconfigure packages/mnemonic-client build system [quick]
...
- [ ] 1. Create `src/client.ts` — MnemonicClient Effect.Service

  **What to do**:
  - Run `bunx effect-solutions show services-and-layers` to confirm Effect.Service patterns
  - Create `src/client.ts` with:
    - `MnemonicClient` as `Effect.Service` using `effect:` (NOT `scoped:`)
    - `dependencies: [FetchHttpClient.layer]`
    - Inside `Effect.gen`: read `Config.string('MNEMONIC_URL')` (default `http://localhost:8787`) and `Config.option(Config.redacted('MNEMONIC_API_KEY'))`
  ...
  **Pattern References**:
  - `src/api.ts` — The `Api` class to import: `HttpApi.make('mnemonic').add(LearningsApi).add(SecretsApi).add(StateApi).add(HealthApi)`
  ...
  - `packages/mnemonic-client/src/index.ts:92-191` — Current MnemonicClient Effect.Service definition (to understand the Config pattern for MNEMONIC_URL and MNEMONIC_API_KEY, then replace the manual HTTP wiring)
  ...
  **Acceptance Criteria**:
  ...
  - [ ] Exports: `MnemonicClient`, `Api`, `Learning`, `InjectResult`, `InjectTraceResult`, `QueryResult`, `Stats`, `Secret`, `WorkingStatePayload`, `WorkingStateResponse`, `NotFoundError`, `ValidationError`, `DatabaseError`, `EmbeddingError`, `Unauthorized`
  ...
  **QA Scenarios:**
  ...
  Scenario: client.ts exports all required symbols
    ...
    Preconditions: src/client.ts created
    Steps:
      1. Run: grep 'export.*MnemonicClient' src/client.ts — expect match
  ...
  **Commit**: YES — groups with Task 2
  - Message: `refactor(client): rewrite mnemonic-client with HttpApiClient for automatic type derivation`
  - Files: `src/client.ts`, `packages/mnemonic-client/*`

- [ ] 2. Reconfigure `packages/mnemonic-client/` build system

  **What to do**:
  - Update `packages/mnemonic-client/package.json`:
  ...
  - Update or create `packages/mnemonic-client/tsup.config.ts` if needed for `dtsResolve` or entry point configuration
  - Update `packages/mnemonic-client/tsconfig.json`:
  ...
  - Rewrite `packages/mnemonic-client/src/index.ts` to a simple re-export: `export * from '../../src/client'`
  ...
  **Must NOT do**:
  ...
  - Do NOT change the package name from `mnemonic-client`
  ...
  **Pattern References**:
  - `packages/mnemonic-client/package.json` — Current package.json to modify (line 19 has build script, line 2 has name, line 3 has version, lines 44-47 have peerDeps)
  - `packages/mnemonic-client/tsconfig.json` — Current tsconfig to modify (line 13 rootDir, line 15 include)
  ...
  **Acceptance Criteria**:
  ...
  - [ ] `bun run build` in `packages/mnemonic-client/` exits 0
  ...
  - [ ] `grep -rl 'drizzle\|libsql\|huggingface\|@effect/sql' packages/mnemonic-client/dist/` returns no matches
  ...
  **QA Scenarios:**
  ...
  Scenario: Build succeeds and dist is clean
    ...
    Steps:
      1. Run: cd packages/mnemonic-client && bun run build
      2. Run: ls packages/mnemonic-client/dist/ — expect .js, .mjs, .d.ts files
      3. Run: grep -rl 'drizzle\|libsql\|huggingface\|@effect/sql' packages/mnemonic-client/dist/ — expect no matches (exit 1)
      4. Run: grep -l 'HttpApiClient' packages/mnemonic-client/dist/*.js — expect match (client code present)
  ...
  Scenario: Package.json is correct
    ...
    Steps:
      1. Run: grep '"version"' packages/mnemonic-client/package.json — expect "0.2.0"
      2. Run: grep '"effect"' packages/mnemonic-client/package.json — expect peerDep entry
      3. Run: grep '"@effect/platform"' packages/mnemonic-client/package.json — expect peerDep entry
...
- [ ] 3. Update README.md
  ...
  **What to do**:
  - Rewrite `packages/mnemonic-client/README.md` to document the new Effect-only API:
    - Installation: `bun add mnemonic-client effect @effect/platform`
    - Basic usage with `MnemonicClient` Effect.Service
    - Show how to provide config (`MNEMONIC_URL`, `MNEMONIC_API_KEY`)
  ...
  - Remove all references to the old `mnemonic()` Promise-based API
  ...
  **Pattern References**:
  - `packages/mnemonic-client/README.md` — Current README to rewrite (documents the old `mnemonic()` API and Promise-based usage)
  ...
  **Acceptance Criteria**:
  ...
  - [ ] No references to `mnemonic()` function or Promise-based API
  - [ ] Shows `MnemonicClient` service usage pattern
  ...
  **QA Scenarios:**
  ...
  Scenario: README documents new API correctly
    ...
    Steps:
      1. Run: grep -c 'mnemonic(' packages/mnemonic-client/README.md — expect 0 (no old API)
      2. Run: grep -c 'await mem\.' packages/mnemonic-client/README.md — expect 0 (no Promise usage)
      3. Run: grep 'MnemonicClient' packages/mnemonic-client/README.md — expect match
      4. Run: grep 'Effect.gen' packages/mnemonic-client/README.md — expect match (shows Effect usage)
      5. Run: grep 'learnings' packages/mnemonic-client/README.md — expect match (documents groups)
...
- [ ] 4. Rewrite smoke test
  ...
  **What to do**:
  - Rewrite `packages/mnemonic-client/test/client.test.ts`:
    - Remove all old tests (they test the deleted `mnemonic()` wrapper)
    - Write a minimal smoke test that:
      1. Spins up a real mnemonic server (same `spawn` + `waitForServer` pattern as existing test)
      2. Creates a `MnemonicClient` layer with config pointing at the test server
    ...
    - Use `Effect.provide` with `FetchHttpClient.layer` and a config provider for `MNEMONIC_URL` + `MNEMONIC_API_KEY`
  ...
  **Must NOT do**:
  ...
  - Do NOT use the old `mnemonic()` wrapper
  ...
  **Pattern References**:
  - `packages/mnemonic-client/test/client.test.ts` — Current test file to rewrite. Reuse the server spawn pattern (lines 57-73: spawn bun, waitForServer), cleanup (lines 75-88: kill + rm), and `uniqueScope()` helper (line 90)
  - `src/client.ts` (Task 1) — The MnemonicClient service to test
  - `packages/mnemonic-client/vitest.config.ts` — Existing vitest config (keep as-is)

  **External References**:
  - Effect testing: `Effect.runPromise(effect.pipe(Effect.provide(layer)))` for running Effects in vitest
  - Config provider: `ConfigProvider.fromMap(new Map([['MNEMONIC_URL', baseUrl], ['MNEMONIC_API_KEY', apiKey]]))`

  **Acceptance Criteria**:

  - [ ] `packages/mnemonic-client/test/client.test.ts` exists and is < 80 lines
  - [ ] Test imports `MnemonicClient` from the new client (not old `mnemonic()` wrapper)
  - [ ] Test verifies at least one endpoint per group (learnings, health minimum)
  - [ ] `bun test` in `packages/mnemonic-client/` passes
  ...
  **QA Scenarios:**
  ...
  Scenario: Smoke test passes end-to-end
    Tool: Bash
    Preconditions: Tasks 1-2 complete, mnemonic server can start
    Steps:
      1. Run: cd packages/mnemonic-client && bun test -- --reporter=verbose
  ...
  Scenario: Test uses new API not old
    ...
    Steps:
      1. Run: grep -c 'mnemonic(' packages/mnemonic-client/test/client.test.ts — expect 0
      2. Run: grep 'MnemonicClient' packages/mnemonic-client/test/client.test.ts — expect match
      3. Run: grep 'Effect' packages/mnemonic-client/test/client.test.ts — expect match
...
- [ ] 5. Full verification — build, types, bundle analysis
  ...
  **What to do**:
  - Run full verification suite to confirm everything works together:
    ...
    2. `cd packages/mnemonic-client && bun run build` — verify tsup build succeeds
    3. Inspect `packages/mnemonic-client/dist/` — verify .js, .mjs, .d.ts, .d.mts files exist
    4. `grep -rl 'drizzle\|libsql\|huggingface\|@effect/sql\|@effect/sql-drizzle\|@effect/sql-libsql' packages/mnemonic-client/dist/` — verify no server-only deps in bundle
    5. `cd packages/mnemonic-client && bun test` — verify smoke test passes
    6. Verify the old manual types are fully gone: `grep -c 'interface Learning\|interface InjectResult\|interface QueryResult\|interface Stats' packages/mnemonic-client/src/index.ts` should return 0 (or file doesn't exist if deleted)
    7. Verify no Promise wrapper: `grep -c 'export function mnemonic\|export default mnemonic' packages/mnemonic-client/src/index.ts src/client.ts` should return 0
  ...
  **QA Scenarios:**
  ...
  Scenario: Full stack verification
    ...
    Steps:
      ...
      2. Run: cd packages/mnemonic-client && bun run build — expect exit 0
      3. Run: ls packages/mnemonic-client/dist/ — expect .js .mjs .d.ts .d.mts files
      4. Run: grep -rl 'drizzle\|libsql\|huggingface\|@effect/sql' packages/mnemonic-client/dist/ — expect exit 1 (no matches)
      5. Run: cd packages/mnemonic-client && bun test — expect exit 0
      6. Run: grep -c 'interface Learning' packages/mnemonic-client/src/index.ts 2>/dev/null || echo 0 — expect 0
      7. Run: grep -c 'export function mnemonic' packages/mnemonic-client/src/index.ts src/client.ts 2>/dev/null || echo 0 — expect 0
...
- [ ] F2. **Code Quality Review** — `unspecified-high`
  Run `tsc --noEmit` + `bun run build` in packages/mnemonic-client. Review `src/client.ts` for: `as any`/`@ts-ignore`, manual type definitions, server-only imports, positional wrapper methods. Check dist/ bundle excludes drizzle/libsql/huggingface. Verify Effect.Service uses `effect:` not `scoped:`.
  Output: `Build [PASS/FAIL] | Types [PASS/FAIL] | Bundle [CLEAN/ISSUES] | VERDICT`

- [ ] F3. **Real Manual QA** — `unspecified-high`
  Start mnemonic server from clean state. Execute smoke test: `cd packages/mnemonic-client && bun test`. Verify the test actually hits the server and round-trips data (not just type-checking). Check test output for real assertion results.
...
- **Single commit**: `refactor(client): rewrite mnemonic-client with HttpApiClient for automatic type derivation`
  - Files: `src/client.ts`, `packages/mnemonic-client/src/index.ts`, `packages/mnemonic-client/package.json`, `packages/mnemonic-client/tsconfig.json`, `packages/mnemonic-client/README.md`, `packages/mnemonic-client/test/client.test.ts`
  - Pre-commit: `cd packages/mnemonic-client && bun run build && bun test`
...
cd packages/mnemonic-client && bun run build        # Expected: exit 0, dist/ populated

# Bundle is clean of server deps
grep -rl 'drizzle\|libsql\|huggingface' packages/mnemonic-client/dist/  # Expected: no matches

# Smoke test passes
cd packages/mnemonic-client && bun test             # Expected: exit 0
```
