...
describe("bulk delete filters", () => {
	it(
		...
		async () => {
			...
			const injectedRows = asArray(asRecord(injected.body).learnings).map((item) =>
				asRecord(item),
			);
...
describe("scope priority", () => {
	it(
		...
		async () => {
			...
			expect(injected.status).toBe(200);
			const ids = asArray(asRecord(injected.body).learnings).map(
				(item) => asRecord(item).id,
			);
...
describe("inject variations", () => {
	...
	it(
		"includeState + runId",
		async () => {
			const scope = memoryScope("inject-include-state");
			const trigger = unique("trigger-include-state");
			await learnMemory({
				scope,
				trigger,
				learning: unique("learning-include-state"),
			});

			const runId = unique("run-XYZ");
			const goal = unique("goal-include-state");
			const putState = await httpJson(
				...
				`/state/${encodeURIComponent(runId)}`,
				{
					...
					body: {
						goal,
						updatedBy: "vitest",
						changeSummary: "seed state for inject includeState",
					},
				},
			);
			...
			expect(injected.status).toBe(200);
			const injectedBody = asRecord(injected.body);
			const prompt = injectedBody.prompt;
			const promptHasState = typeof prompt === "string" && prompt.includes("Working state (live):");

			const stateValue = injectedBody.state;
			const stateRunId =
				typeof stateValue === "object" && stateValue !== null
					? asRecord(stateValue).runId
					: undefined;

			expect(promptHasState || stateRunId === runId).toBe(true);
		},
		TEST_TIMEOUT_MS,
	);
});
