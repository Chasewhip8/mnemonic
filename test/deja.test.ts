...
describe("learn + inject/query/trace", () => {
	it(
		...
		async () => {
			...
			expect(learnings.some((item) => asRecord(item).id === learnedId)).toBe(
				true,
			);
		},
		TEST_TIMEOUT_MS,
...
describe("learning endpoints", () => {
	it(
		...
		async () => {
			...
			expect(
				asArray(listed.body).some((item) => asRecord(item).id === learnedId),
			).toBe(true);
			...
			expect(
				asArray(listedAfterDelete.body).some(
					(item) => asRecord(item).id === learnedId,
				),
			).toBe(false);
		},
		TEST_TIMEOUT_MS,
	...
	it(
		...
		async () => {
			...
			const remainingIds = asArray(remaining.body).map(
...
describe("secrets", () => {
	it(
		...
		async () => {
			...
			expect(
				asArray(listSecrets.body).some((item) => asRecord(item).name === name),
			).toBe(false);
		},
		TEST_TIMEOUT_MS,
	);
...
describe("state lifecycle", () => {
	...
	it(
		...
		async () => {
			...
			expect(
				rows.some((row) => row.trigger === `run:${runId} resolved`),
			).toBe(true);
		},
		TEST_TIMEOUT_MS,
	);
